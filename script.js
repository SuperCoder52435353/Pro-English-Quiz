...
let quizSettings = JSON.parse(localStorage.getItem('quizSettings')) || { maxAttempts: 3, timeLimit: 30 };
...
function checkForm() {
    ...
    // Agar Abduraxmon Admin bo‘lsa, parol so‘rash
    if (isAdmin) {
        ...
        // Online foydalanuvchilarni saqlash
        onlineUsers[`${firstName} ${lastName}`] = { status: 'online', currentQuiz: '', progress: '', lastActive: Date.now() };
...
function startQuiz(level) {
    ...
    const userAttempts = allResults.filter(result => result.user === userKey && result.level === level).length;
    if (userAttempts >= quizSettings.maxAttempts) {
        alert(`Sizda ${level} darajasida urinishlar soni tugadi!`);
        return;
    }
    ...
    usedWords[userKey] = usedWords[userKey] || {};
    usedWords[userKey][level] = usedWords[userKey][level] || [];
    const availableWords = wordsBase[level].filter(word => !usedWords[userKey][level].includes(word.word));
    if (availableWords.length < totalQuestions) {
        usedWords[userKey][level] = []; // Agar so‘zlar tugasa, qaytadan boshlash
    ...
    // Online foydalanuvchi ma'lumotini yangilash
    onlineUsers[userKey] = { status: 'online', currentQuiz: level, progress: `${currentQuestion + 1}/${totalQuestions}`, lastActive: Date.now() };
    localStorage.setItem('onlineUsers', JSON.stringify(onlineUsers));
...
function showQuestion() {
    ...
    if (currentQuestion < totalQuestions) {
        ...
        // Online foydalanuvchi ma'lumotini yangilash
        const userKey = `${userData.firstName} ${userData.lastName}`;
        onlineUsers[userKey] = { status: 'online', currentQuiz: currentQuiz, progress: `${currentQuestion + 1}/${totalQuestions}`, lastActive: Date.now() };
        localStorage.setItem('onlineUsers', JSON.stringify(onlineUsers));
    } else {
        finishQuiz();
    }
...
function nextQuestion() {
    ...
    usedWords[userKey][currentQuiz].push(currentWord);
...
function finishQuiz() {
    ...
    // Online foydalanuvchi ma'lumotini yangilash
    onlineUsers[userKey] = { status: 'online', currentQuiz: '', progress: '', lastActive: Date.now() };
    localStorage.setItem('onlineUsers', JSON.stringify(onlineUsers));
...
function updateOnlineUsers() {
    ...
    const onlineUsersList = Object.entries(onlineUsers).map(([user, info]) => {
        const elapsed = Date.now() - info.lastActive;
        if (elapsed > 60000) { // 1 daqiqadan ko‘p bo‘lsa offline
            delete onlineUsers[user];
            localStorage.setItem('onlineUsers', JSON.stringify(onlineUsers));
            return '';
        }
        return `<div>User: ${user}<br>Status: ${info.status}<br>Current Quiz: ${info.currentQuiz || 'Yo‘q'}<br>Progress: ${info.progress || 'Yo‘q'}</div>`;
    }).filter(entry => entry).join('');
    document.getElementById('onlineUsers').innerHTML = onlineUsersList || 'Hozirda online foydalanuvchilar yo‘q.';

    setTimeout(updateOnlineUsers, 5000); // Har 5 soniyada yangilash
}
